...
from robosuite.wrappers.gym_wrapper_r3m import GymWrapperR3M
...
def make_env(env_params: dict, rank: int = 0, seed: int = 0) -> Callable:
    ...
    def _init() -> gym.Env:
        if env_params["use_camera_obs"]:
            # These parameters are needed to use R3M embeddings
            ...
            env = Monitor((GymWrapperR3M(suite.make(**env_params), keys=['agentview_image','agentview_depth'])))
        else:
...
if __name__ ==  '__main__':
    ...
    # Not needed as we don't observe images
